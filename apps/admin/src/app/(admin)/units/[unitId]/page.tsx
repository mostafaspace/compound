...
import { getApartmentDetail, getCurrentUser, getUnit, listPenaltyPoints } from "@/lib/api";
import { requireAdminUser } from "@/lib/session";

import { endMembershipAction, inviteResidentToUnitAction, updateMembershipProfileAction } from "../actions";
import { PenaltyPointsPanel } from "./penalty-points-panel";
...
export default async function UnitDetailPage({ params, searchParams }: UnitDetailPageProps) {
  ...
  const [unit, t, apartment, penaltyPoints] = await Promise.all([
    getUnit(unitId),
    getTranslations("Registry"),
    getApartmentDetail(unitId),
    listPenaltyPoints(unitId),
  ]);
      ...
      {apartment ? (
        <section className="mx-auto max-w-7xl space-y-5 px-5 py-6 lg:px-8">
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            ...
            <PenaltyPointsPanel unitId={unit.id} initialEvents={penaltyPoints} />
          </div>
        </section>
      ) : null}
