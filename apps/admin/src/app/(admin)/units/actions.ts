...
export async function addPenaltyAction(unitId: string, data: { points: number; reason: string; notes?: string; expires_at?: string }) {
  await addPenaltyPoints(unitId, data);
  revalidatePath(`/units/${unitId}`);
}

export async function voidPenaltyAction(unitId: string, eventId: number, reason: string) {
  await voidPenaltyPoint(eventId, reason);
  revalidatePath(`/units/${unitId}`);
}
