...
### Rendered Lists And Paging

- All app UI lists must use a virtualized list primitive (`FlatList`, `SectionList`, or an approved wrapper). Do not render repeated UI rows/cards with `.map()` in screens.
- Backend endpoints that can grow must expose paged/chunked responses. Mobile clients should use `FlatList` with `onEndReached`, `refreshing`, stable keys, and explicit loading/empty/error states.
- `.map()` is acceptable only for bounded data transformation outside render output, not for visible repeated rows.
